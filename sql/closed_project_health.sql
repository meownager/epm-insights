-- Closed Project Health Analysis
-- Computes proposed-vs-actual deviation metrics for closed or completed projects.
--
-- Expected DuckDB views or tables:
--   proposal_projects
--   actual_projects

WITH closed_projects AS (
  SELECT
    p.project_id,
    p.project_name,
    p.client,
  ...
  WHERE LOWER(TRIM(COALESCE(a.status, ''))) IN ('closed', 'completed')
),
metrics AS (
  SELECT
    *,
